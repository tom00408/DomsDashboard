	...
	<div class="produkte-container">
		...
		<form
			...
			<input
				...
				placeholder="Bildpfad (URL) oder Datei auswählen" />
			<input type="file" @change="handleImageUpload" accept="image/*" />
			<input
				v-model="newProduct.sizes"
				placeholder="Größen (Komma-getrennt)" />
			<div class="checkbox-row">
				<label class="custom-checkbox"
					><input type="checkbox" v-model="newProduct.hasName" /><span
						>Hat Name</span
					></label
				>
				<label class="custom-checkbox"
					><input
						type="checkbox"
						v-model="newProduct.hasNumber" /><span
						>Hat Nummer</span
					></label
				>
				<label class="custom-checkbox"
					><input
						type="checkbox"
						v-model="newProduct.hasInitials" /><span
						>Hat Initialen</span
					></label
				>
				<label class="custom-checkbox"
					><input
						type="checkbox"
						v-model="newProduct.isUnisex" /><span
						>Ist Unisex (sonst männlich oder weiblich)</span
					></label
				>
			</div>
		...
		<div class="produkt-liste">
			...
			<div
				...
				<div class="image-container">
					<img
						v-if="produkt.imageUrl"
						:src="produkt.imageUrl"
						alt="Bild"
						class="produkt-image"
						@error="handleImageError" />
					<div v-else class="produkt-image-placeholder">
						Kein Bild
					</div>
				</div>
				<div class="produkt-info">
					...
					<div>
						Größen:
						{{ produkt.size ? produkt.size.join(', ') : '' }}
					</div>
					<div class="produkt-flags">
						<span v-if="produkt.hasName" class="flag"
							>🟢 hat Name</span
						>
						<span v-else class="flag">🔴 kein Name</span>
						<span v-if="produkt.hasNumber" class="flag"
							>🟢 hat Nummer</span
						>
						<span v-else class="flag">🔴 keine Nummer</span>
						<span v-if="produkt.hasInitials" class="flag"
							>🟢 hat Initialen</span
						>
						<span v-else class="flag">🔴 keine Initialen</span>
						<span v-if="produkt.isUnisex" class="flag"
							>🟢 ist Unisex</span
						>
						<span v-else class="flag">🔴 ist nicht Unisex</span>
					</div>
				...
				<div class="produkt-actions">
					...
					<button
						@click="deleteProduct(produkt.id)"
						class="delete-btn">
						Löschen
					</button>
				</div>
		...
		<form
			...
			<input
				...
				required />
			<input
				v-model="editProductData.imagepath"
				placeholder="Bildpfad (URL) oder Datei auswählen" />
			<input
				type="file"
				...
				accept="image/*" />
			<input
				v-model="editProductData.sizes"
				placeholder="Größen (Komma-getrennt)" />
			<div class="checkbox-row">
				<label class="custom-checkbox"
					><input
						type="checkbox"
						v-model="editProductData.hasName" /><span
						>Hat Name</span
					></label
				>
				<label class="custom-checkbox"
					><input
						type="checkbox"
						v-model="editProductData.hasNumber" /><span
						>Hat Nummer</span
					></label
				>
				<label class="custom-checkbox"
					><input
						type="checkbox"
						v-model="editProductData.hasInitials" /><span
						>Hat Initialen</span
					></label
				>
				<label class="custom-checkbox"
					><input
						type="checkbox"
						v-model="editProductData.isUnisex" /><span
						>Ist Unisex (sonst männlich oder weiblich)</span
					></label
				>
			</div>
...
const loadProdukte = async () => {
	...
	const produktArr = querySnapshot.docs.map((doc) => ({
		id: doc.id,
		...doc.data(),
	}));
...
const addProduct = async () => {
	const produkt = {
		...
		isUnisex: newProduct.value.isUnisex,
	};
...
const editProduct = (produkt) => {
	editProductData.value = {
		...
		isUnisex: produkt.isUnisex ?? false,
	};
	editForm.value = true;
};
...
const updateProduct = async () => {
	const produkt = {
		...
		isUnisex: editProductData.value.isUnisex,
	};
...
const handleImageError = (event) => {
	...
	if (
		placeholder &&
		placeholder.classList.contains('produkt-image-placeholder')
	) {
